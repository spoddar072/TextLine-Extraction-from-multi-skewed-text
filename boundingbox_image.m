...
close all;
clc;
img = imread('D:\oil drop\input Images\images_train\016.tif');
...
for p = 1 : ccNo
 ...
 %    rectangle('Position', compCoord, 'EdgeColor', 'r');
    x1 = floor(compCoord(1)); %ceil(stats(1).BoundingBox;
    y1 = floor(compCoord(2)); %ceil(stats(2).BoundingBox;
    width = compCoord(3); %stats(3).BoundingBox;
    height = compCoord(4); %stats(4).BoundingBox;
    y2 = y1 + height;
    x2 = x1 + width;
    for j = 1 : 2
        info1(p, j) = floor(stats(p).BoundingBox(1, j));        
    end
    info1(p, 3) = x2;
    info1(p, 4) = y1;
    info1(p, 5) = x2;
...
num = largeComps;
display(num);
% completely_bounded = zeros(1, num);
% for i = 1 : num
%     for j = 1 : num
%         if info(j, xul) > info(i, xul) && info(j, xur) < info(i, xur) && info(j, yul) > info(i, yul) && info(j, yll) < info(i, yll)
%             completely_bounded(j) = 1;
%         end
%     end
% end
% hold on;
% figure,imshow(img2);
% temp_info = zeros(num, 12);
% count = 0;
% for i = 1 : num
%     if completely_bounded(i) == 0
%         count = count + 1;
%         compCoord = stats(i).BoundingBox;
%         rectangle('Position', compCoord, 'EdgeColor', 'r');
%         temp_info(i, 1 : 10) = info(i, 1 : 10);
%         temp_info(i, lbl) = count;
%         temp_info(i, area) = info(i, area);
%     end
% end
% hold off;
% num = count;
...
for i=1:1:num
    for j=info(i,yul):1:info(i,yll)
        for k=info(i,xul):1:info(i,xur)
%             if img2(j,k)==0
